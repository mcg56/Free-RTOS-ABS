...
if (xTimer == xTimer_ReadABS){
    ulMaxExpiryCountBeforeStopping = 500;
}
if (xTimer == xTimer_Delay){
    ulMaxExpiryCountBeforeStopping = 4;
}
    ...
    if (xTimer == xTimer_ReadABS)
    {
        ...
        while (( uint32_t ) pvTimerGetTimerID( xTimer_Delay ) <= 1)
        {
            brakeOnCount++;
    ...
    if (ulCount % 20 == 0 && xTimer == xTimer_ReadABS)
    {
        ...
        //ulCount_delay = ( uint32_t ) pvTimerGetTimerID( xTimer_Delay );
    ...
    //     {
    //         OLEDStringDraw ("High Edge found", 0, 2);
    //     } else:
    //     OLEDStringDraw ("High Edge found", 0, 2);
...
int main(void) {
    ...
    xTimer_Delay = xTimerCreate
                   ( /* Just a text name, not used by the RTOS
                     kernel. */
                     "Delay Timer",
                     /* The timer period in ticks, 1/500s */
                     pdMS_TO_TICKS(2) ,
                     /* The timers will auto-reload themselves
                     when they expire. */
                     pdTRUE,
